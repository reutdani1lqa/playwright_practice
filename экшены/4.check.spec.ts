...
test.describe('Комплексное тестирование формы с чекбоксами', () => {
  ...
  test('Полное заполнение формы с проверкой состояний', async ({ page }) => {
    // Чекбоксы
    const mailingCheckbox = page.getByLabel(' Подписаться на рассылку ');
    await mailingCheckbox.check();
    const termsOfAgreementCheckbox = page.getByLabel(' Я принимаю условия соглашения ');
    await termsOfAgreementCheckbox.check();
    // Группа интересов
    ...
    // Радио-кнопки
    const diliveryRadiobutton = page.getByLabel(' Почта России ');
    await diliveryRadiobutton.check();
    // Кастомный элемент
    await page.locator('.tos-container').scrollIntoViewIfNeeded();
    const termsOfUseCheckbox = page.getByLabel(' Я прочитал и согласен с условиями ');
    await termsOfAgreementCheckbox.check();
    // Проверки
